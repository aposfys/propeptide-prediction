'''
Generate ProstT5 SEQUENCE-ONLY per-residue embeddings, one file per sequence.

Renamed from make_embeddings.py when the three paper arms were consolidated onto
one branch. `make_embeddings.py` here is the ESM3 extractor; this is ProstT5's.
Keeping both under one name was the fastest way to run two arms on code that
only looked identical.
