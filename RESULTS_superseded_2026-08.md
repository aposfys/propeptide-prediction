...
> **Under revision — do not cite from this file yet.**
>
> Several figures below are known to be wrong and are pending retraction:
> the "+0.060 over its own control / 3.3 sigma" fine-tuning claim divides a
> fine-tuned mean by the *frozen* arm's standard deviation (0.0185); against the
> fine-tuned arm's own spread it is about one sigma. Comparisons that pool runs
> across different learning rates treat a validation-selected hyperparameter as
> replicate noise. Numbers are also mixed across two metric versions and across
> arms given unequal hyperparameter tuning.
>
> A replicated, equal-budget comparison across representations is running now.
> This file will be regenerated from `test_metrics.json` / `valid_metrics.json`
> by `src/utils/summarize_results.py`, with nothing carried over by hand.
