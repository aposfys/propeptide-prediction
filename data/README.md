...
The folds are indicated in `graphpart_assignments.csv`, which is not on this archived branch; it is on `main`. Column `cluster` indicates the fold. All sequences that are not listed in this file are not part of the 7,623 proteins that were used for training and evaluation.