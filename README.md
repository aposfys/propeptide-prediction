...
First derive 3Di from AlphaFold models, then extract:
...
`--fuse renorm` holds the input at 1024 dims, so the head is identical to the
sequence-only control rather than 78% wider. Its own control:

```bash
python -m src.utils.make_embeddings_prost5 data/protein_sequences.fasta \\
    embeddings/prost5_seqonly
```

`--shuffle_3di` is the negative control: same dimensions, same mask, wrong
structures.

```bash
ARM=prost5_fused EMB=embeddings/prost5_fused DIM=1024 bash run_arm.sh 8
```
