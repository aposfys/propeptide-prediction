...
class CRFBaseModel(nn.Module):
    ...
    def _esm_embed(sequence: str, device: torch.device, repr_layers: int = 33) -> torch.Tensor:
        ...
        Uses a sliding window (1022 tokens, 300-token overlap) to handle sequences
        longer than the ESM-2 context limit.
        '''
        ...
        repr_layers_list = [
            (i + esm_model.num_layers + 1) % (esm_model.num_layers + 1)
            for i in range(repr_layers)
        ]
        ...
        while end <= minibatch_max_length:
            ...
            if end <= minibatch_max_length:
                end = end - 300  # 300-token overlap so context isn't lost at boundaries
            tokens = esm_model(
                toks[:, start:end],
        ...
        out[out != out] = 0.0  # replace NaN with zero

        res = out.transpose(0, 1)[1:-1]  # strip CLS/EOS → (L, batch, dim)
        seq_embedding = res[:, 0]        # single sequence → (L, 1280)
...
class LSTMCNNCRF(CRFBaseModel):
    ...
    def _init_weights(self) -> None:
        ...
        # Conv1: compresses ESM-2 1280-dim → n_filters; kaiming for ReLU
